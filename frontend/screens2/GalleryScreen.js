...
import { View, FlatList, Image, StyleSheet, ActivityIndicator, Text, Alert, TouchableOpacity, Modal } from 'react-native';
...
export default function GalleryScreen() {
  ...
  const handleDelete = async (url) => {
    try {
      ...
      Alert.alert('Delete Error', 'Failed to delete the media. Please try again.');
    }
  };
  ...
  const confirmDelete = (url) => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Delete', style: 'destructive', onPress: () => handleDelete(url) }
      ]
    );
  };
  ...
  return (
    <View style={styles.container}>
      {loading ? (
        ...
        <Text style={styles.errorText}>Empty gallery, please login and access images.</Text>
      ) : error ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.mediaContainer}>
              ...
              <TouchableOpacity onPress={() => confirmDelete(item.url)} style={styles.deleteButton}>
